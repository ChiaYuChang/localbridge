...
- **External Interfaces**: MCP over OpenAI Tunnel (`tunnelclient.New`, `client.Start`, `WaitUntilReady`); in-memory transports (`mcp.NewInMemoryTransports`, `server.Run`); tools `echo`, `read_file`, `read_partial_file`; env `CONTROL_PLANE_TUNNEL_ID`, `CONTROL_PLANE_API_KEY`/`OPENAI_API_KEY`, `CONTROL_PLANE_BASE_URL`, `CONTROL_PLANE_ORGANIZATION_ID`, `CONTROL_PLANE_POLL_TIMEOUT`, `CONTROL_PLANE_EXTRA_HEADERS`, `TUNNEL_CLIENT_SDK_READY_FILE`.
...
- **Read Limits**: `read_file` max 4MB (`MaxFileSize`), `read_partial_file` max 1MB (`MaxPartialFileSize`), `MaxBatchFiles=50`; UTF-8 enforced (`utf8.Valid`); dir rejected; `start_line>=1`, `end_line>=start_line`, `start_line` beyond EOF errors.
...
- **Tool Contract**: Input/output structs with `json` tags (`ToolEchoI`, `ToolEchoO`, `ToolReadFileI/O`, `ToolReadPartialFileI/O`); `Name()` unique (`echo`, `read_file`, `read_partial_file`); `Register(*mcp.Server) error` via `mcp.AddTool`; handler returns `(CallToolResult, Output, error)`.
...
- **Product Checks**: `os.Root` escape attempts blocked; size/UTF-8/dir guards hit; `start_line`/`end_line` edge cases; dual-entry drift (`main.go` vs `cmd/main.go`) flagged; unregistered filesystem tools (`read_file` not wired in `run()`) flagged as scope gap; empty stubs flagged; `0600` ready-file, no secret leak, `.secrets/` untracked.
...
- **Observed-At**: `2026-09-14T14:57:49Z @ a8e75904 (change tvktruox)`
- **Dirty Status**: `dirty (21 adds unsealed incl opencode.json vault-allow; SCAN_CLEAN; commit flow Step 6 done)`
- **Milestone**: `COMMIT_FLOW_AUTH_GATE - candidate stabilized, AGENTS.md barrier check pending, seal awaiting explicit auth`
- **Next Pickup Item**: `Reviewer AGENTS_REVIEW_PASS/BARRIER_LEAK verdict, then present message for seal auth`
