...
- **External Interfaces**: MCP over OpenAI Tunnel (`tunnelclient.New`, `client.Start`, `WaitUntilReady`); in-memory transports (`mcp.NewInMemoryTransports`, `server.Run`); tools `echo`, `read_file`; env `OPENAI_TUNNEL_ID(_FILE)`, `OPENAI_API_KEY(_FILE)` (single general key, also authorizing tunnel), `OPENAI_TUNNEL_BASE_URL`, `OPENAI_TUNNEL_ORGANIZATION_ID`, `OPENAI_TUNNEL_POLL_TIMEOUT`, `OPENAI_TUNNEL_EXTRA_HEADERS`, `TUNNEL_CLIENT_SDK_READY_FILE`.
...
- **Observed-At**: `2026-09-19T05:51:17Z - fix-forward incl validate-only pivot unsealed (seal pending)`
- **Dirty Status**: `dirty (secret-home + CI + validate-only unsealed; IMPLEMENTATION_REVIEW_PASS hardened; DEFEND_PASS; SCAN pending)`
- **Milestone**: `COMMIT_FLOW_STEP_7 - seal message presented, awaiting auth`
- **Next Pickup Item**: `User auth (ok) -> jj commit (local only, no push); retro-gates close on seal`
