...
- **Toolchain**: Go 1.27.0 via host toolchain (`go.mod`; Docker `golang:1.27.0-bookworm`, CI `setup-go` pinned); devenv retired (no `devenv.*` files; `nix-profile` Go acceptable); `.secrets/` gitignored, never copy/mount operator secrets into sandbox; generated creds local-only throwaway.
