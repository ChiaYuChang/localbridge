...
- **External Interfaces**: MCP over OpenAI Tunnel (`tunnelclient.New`, `client.Start`, `WaitUntilReady`); in-memory transports (`mcp.NewInMemoryTransports`, `server.Run`); 17 natives via `gateway.nativeTools` (8 fs `read_file`, `read_multiple_files`, `search_files`, `search_within_files`, `list_directory`, `tree`, `get_file_info`, `list_allowed_directories`; 4 git `git_status`, `git_diff`, `git_log`, `git_show`; 4 jj `jj_status`, `jj_diff`, `jj_log`, `jj_show`; `echo`); env `OPENAI_TUNNEL_ID(_FILE)`, `OPENAI_API_KEY(_FILE)` (single general key, also authorizing tunnel), `OPENAI_TUNNEL_BASE_URL`, `OPENAI_TUNNEL_ORGANIZATION_ID`, `OPENAI_TUNNEL_POLL_TIMEOUT`, `OPENAI_TUNNEL_EXTRA_HEADERS`, `TUNNEL_CLIENT_SDK_READY_FILE`.
- **Module Map**: Entry `cmd/main.go` (`run()`: flags/env, `gateway.Compose`, tunnel attach; tests `cmd/main_test.go`); composition `internal/gateway/gateway.go` (`nativeTools` frozen 8+4+4+echo, staged Compose/Serve); contract `internal/tools/tools.go` (`Tool{Name,Register}`); impl `internal/tools/test/echo.go` (`ToolEcho`); sandbox `internal/tools/filesystem/filesystem.go` (`os.OpenRoot`, `FileSystem{root *os.Root}`); readers `internal/tools/filesystem/file.go`; metadata `directory.go`/`allowed.go`; search `search.go`; hider wiring `hider.go`; git `internal/tools/git/` (`status/diff/log/show` via `os/exec`); jj `internal/tools/jj/` (`status/diff/log/show` via `os/exec` + fake fixture); secrets `internal/tools/secrets/secrets.go` (unified `Secret`/`SecretHider`, `Enable` Extra-only, `Lead`, `Compile`); zero empty stubs; tests co-located `*_test.go`. Retired: root `main.go` (broken `internal/tools/echo` import, superseded).
...
- **Tool Contract**: Input/output structs with `json` tags (per-tool `*I/O`, e.g. `ToolEchoI/O`, `ToolReadFileI/O`); `Name()` unique (17 natives); `Register(*mcp.Server) error` via `mcp.AddTool`; handler returns `(CallToolResult, Output, error)`.
...
- **Observed-At**: `2026-09-23T11:39:44Z - installer + AGENTS drift fix sealed; working clean`
- **Dirty Status**: `clean`
- **Milestone**: `COMMIT_SEALED - feat(installer): rustup toolchain + cargo budget; local only, no push`
- **Next Pickup Item**: `Phase 2 admin tools; per-tool toggles parked; rustfs live pending (RUST_LOG fix in place)`
